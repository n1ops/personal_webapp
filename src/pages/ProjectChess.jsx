...
import './ProjectChess.css';

const project = projects.find((p) => p.id === 'chess');

export default function ProjectChess() {
    ...
    <PageTransition>
      <div className="project-deep">
        ...
        <div className="project-body container">
          ...
          <SectionBlock title="Project Layout">
            <div className="file-tree">
              {project.projectLayout.map((line, i) => {
                const commentMatch = line.match(/(.*)(#.*)$/);
                if (commentMatch) {
                  return (
                    <div key={i} className="file-tree-line">
                      {commentMatch[1]}
                      <span className="file-tree-comment">{commentMatch[2]}</span>
                    </div>
                  );
                }
                return (
          ...
          <SectionBlock title="Design Principles">
            <div className="feature-list">
              {project.designPrinciples.map((d, i) => (
                <Reveal key={d} delay={i * 0.05} direction="left">
          ...
          <SectionBlock title="Roadmap">
            <div className="roadmap-list">
              {project.roadmap.map((r, i) => (
                <Reveal key={r} delay={i * 0.05}>
                  <div className="roadmap-item">
                    <span className="roadmap-item-indicator" />
                    {r}
                  </div>
                </Reveal>
              ))}
